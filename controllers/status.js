...
export const getMounthSum = (req, res) => {
  ...
  db.query(currentMonthQuery, (err, result) => {
    if (err) {
      console.error('Erro ao calcular dados do mês atual:', err);
      return res.status(500).json({ error: 'Erro ao calcular dados do mês atual.' });
    }

    return res.status(200).json({
      totalMesAtual: result[0].totalMesAtual || 0,
      quantidadeMesAtual: result[0].quantidadeMesAtual || 0,
      mediaMesAtual: result[0].mediaMesAtual || 0,
    });
  });
};

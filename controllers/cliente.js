        ...
        db.query(insertContatoQuery, [con_telefone], (err, contatoResult) => {
            ...
            db.query(insertClienteQuery, [cli_nome, cli_sobrenome, enderecoId, contatoId, empresa_fk, cli_numero, cli_complemento], (err, result) => {
                ...
                const selectQuery = `
                    ...
                    WHERE cli.cli_id = ?
                `;
                db.query(selectQuery, [result.insertId], (err2, data) => {
                    if (err2) {
                        console.error("Erro ao buscar cliente inserido:", err2);
                        return res.status(500).send("Erro ao cadastrar cliente. Tente novamente.");
                    }
...
export const getPhoneSuggestions = (req, res) => {
    const { partialTelefone } = req.params;

    const q = `
        SELECT DISTINCT con.con_telefone
        FROM con_contato con
        WHERE con.con_telefone LIKE ? 
        LIMIT 10
    `;

    db.query(q, [`%${partialTelefone}%`], (err, data) => {
        if (err) return res.json(err);
        return res.status(200).json(data.map(row => row.con_telefone));
    });
};
